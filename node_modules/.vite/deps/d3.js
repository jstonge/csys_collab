...
function cumsum(values, valueof) {
  var sum4 = 0, index2 = 0;
  return Float64Array.from(values, valueof === void 0 ? (v2) => sum4 += +v2 || 0 : (v2) => sum4 += +valueof(v2, index2++, values) || 0);
}
      ...
      if (step > 0) {
        for (i = 0; i < n; ++i) {
          if ((x2 = values[i]) != null && x06 <= x2 && x2 <= x12) {
            bins[Math.min(m2, Math.floor((x2 - x06) / step))].push(data[i]);
          }
        }
      } else if (step < 0) {
        for (i = 0; i < n; ++i) {
...
function gamma(y2) {
  return (y2 = +y2) === 1 ? nogamma : function(a3, b) {
    return b - a3 ? exponential(a3, b, y2) : constant_default3(isNaN(a3) ? b : a3);
  };
}
...
function genericArray(a3, b) {
  var nb = b ? b.length : 0, na = a3 ? Math.min(nb, a3.length) : 0, x2 = new Array(na), c5 = new Array(nb), i;
  for (i = 0; i < na; ++i)
...
var Path = class {
  ...
  arcTo(x12, y12, x2, y2, r) {
    ...
    if (this._x1 === null) {
      ...
      let x20 = x2 - x06, y20 = y2 - y06, l21_2 = x21 * x21 + y21 * y21, l20_2 = x20 * x20 + y20 * y20, l21 = Math.sqrt(l21_2), l01 = Math.sqrt(l01_2), l = r * Math.tan((pi3 - Math.acos((l21_2 + l01_2 - l20_2) / (2 * l21 * l01))) / 2), t01 = l / l01, t21 = l / l21;
  ...
  arc(x2, y2, r, a0, a1, ccw) {
    ...
    if (r < 0)
      throw new Error(`negative radius: ${r}`);
    let dx = r * Math.cos(a0), dy = r * Math.sin(a0), x06 = x2 + dx, y06 = y2 + dy, cw = 1 ^ ccw, da2 = ccw ? a0 - a1 : a1 - a0;
    if (this._x1 === null) {
    ...
    if (da2 > tauEpsilon) {
      ...
      this._append`A${r},${r},0,${+(da2 >= pi3)},${cw},${this._x1 = x2 + r * Math.cos(a1)},${this._y1 = y2 + r * Math.sin(a1)}`;
    }
  }
  rect(x2, y2, w, h) {
...
function contours_default() {
  ...
  function smoothLinear(ring, values, value) {
    ring.forEach(function(point6) {
      var x2 = point6[0], y2 = point6[1], xt = x2 | 0, yt = y2 | 0, v1 = valid(values[yt * dx + xt]);
      if (x2 > 0 && x2 < dx && xt === x2) {
        point6[0] = smooth1(x2, valid(values[yt * dx + xt - 1]), v1, value);
      }
      if (y2 > 0 && y2 < dy && yt === y2) {
        point6[1] = smooth1(y2, valid(values[(yt - 1) * dx + xt]), v1, value);
      }
    });
  }
...
function density_default() {
  var x2 = defaultX, y2 = defaultY, weight = defaultWeight, dx = 960, dy = 500, r = 20, k2 = 2, o = r * 3, n = dx + o * 2 >> k2, m2 = dy + o * 2 >> k2, threshold2 = constant_default6(20);
  function grid(data) {
...
var Voronoi = class {
  ...
  *neighbors(i) {
  ...
  _project(x06, y06, vx, vy) {
    let t = Infinity, c5, x2, y2;
    if (vy < 0) {
      if (y06 <= this.ymin)
        return null;
      if ((c5 = (this.ymin - y06) / vy) < t)
        y2 = this.ymin, x2 = x06 + (t = c5) * vx;
    } else if (vy > 0) {
      if (y06 >= this.ymax)
        return null;
      if ((c5 = (this.ymax - y06) / vy) < t)
        y2 = this.ymax, x2 = x06 + (t = c5) * vx;
    }
    if (vx > 0) {
      if (x06 >= this.xmax)
        return null;
      if ((c5 = (this.xmax - x06) / vx) < t)
        x2 = this.xmax, y2 = y06 + (t = c5) * vy;
    } else if (vx < 0) {
      if (x06 <= this.xmin)
        return null;
      if ((c5 = (this.xmin - x06) / vx) < t)
        x2 = this.xmin, y2 = y06 + (t = c5) * vy;
    }
    return [x2, y2];
  }
  _edgecode(x2, y2) {
...
var Delaunay = class {
  ...
  *neighbors(i) {
  ...
  find(x2, y2, i = 0) {
    ...
    const i0 = i;
    let c5;
    while ((c5 = this._step(i, x2, y2)) >= 0 && c5 !== i && c5 !== i0)
      i = c5;
    return c5;
  }
  _step(i, x2, y2) {
...
function centroidRingPoint(lambda, phi2) {
  lambda *= radians2, phi2 *= radians2;
  var cosPhi = cos2(phi2), x2 = cosPhi * cos2(lambda), y2 = cosPhi * sin2(lambda), z = sin2(phi2), cx = y0 * z - z0 * y2, cy = z0 * x2 - x0 * z, cz = x0 * y2 - y0 * x2, m2 = hypot(cx, cy, cz), w = asin(m2), v2 = m2 && -w / m2;
  X2.add(v2 * cx);
  Y2.add(v2 * cy);
  Z2.add(v2 * cz);
  W1 += w;
...
function centroid_default(object2) {
  ...
  var x2 = +X2, y2 = +Y2, z = +Z2, m2 = hypot(x2, y2, z);
  if (m2 < epsilon22) {
    x2 = X1, y2 = Y1, z = Z1;
    if (W1 < epsilon7)
      x2 = X0, y2 = Y0, z = Z0;
  ...
  return [atan2(y2, x2) * degrees3, asin(z / m2) * degrees3];
}
...
function graticuleX(y06, y12, dy) {
  var y2 = range(y06, y12 - epsilon7, dy).concat(y12);
  return function(x2) {
...
function graticuleY(x06, x12, dx) {
  var x2 = range(x06, x12 - epsilon7, dx).concat(x12);
  return function(y2) {
...
function interpolate_default2(a3, b) {
  ...
  var interpolate = d ? function(t) {
    var B3 = sin2(t *= d) / k2, A5 = sin2(d - t) / k2, x2 = A5 * kx0 + B3 * kx1, y2 = A5 * ky0 + B3 * ky1, z = A5 * sy0 + B3 * sy1;
    return [
...
function resample(project, delta2) {
  function resampleLineTo(x06, y06, lambda04, a0, b02, c0, x12, y12, lambda12, a1, b12, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a3 = a0 + a1, b = b02 + b12, c5 = c0 + c1, m2 = sqrt(a3 * a3 + b * b + c5 * c5), phi2 = asin(c5 /= m2), lambda22 = abs3(abs3(c5) - 1) < epsilon7 || abs3(lambda04 - lambda12) < epsilon7 ? (lambda04 + lambda12) / 2 : atan2(b, a3), p = project(lambda22, phi2), x2 = p[0], y2 = p[1], dx2 = x2 - x06, dy2 = y2 - y06, dz = dy * dx2 - dx * dy2;
      if (dz * dz / d2 > delta2 || abs3((dx * dx2 + dy * dy2) / d2 - 0.5) > 0.3 || a0 * a1 + b02 * b12 + c0 * c1 < cosMinDistance) {
        resampleLineTo(x06, y06, lambda04, a0, b02, c0, x2, y2, lambda22, a3 /= m2, b /= m2, c5, depth, stream);
        stream.point(x2, y2);
        resampleLineTo(x2, y2, lambda22, a3, b, c5, x12, y12, lambda12, a1, b12, c1, depth, stream);
      }
    }
  }
  return function(stream) {
...
function projectionMutator(projectAt) {
  var project, k2 = 150, x2 = 480, y2 = 250, lambda = 0, phi2 = 0, deltaLambda = 0, deltaPhi = 0, deltaGamma = 0, rotate, alpha = 0, sx = 1, sy = 1, theta = null, preclip = antimeridian_default, x06 = null, y06, x12, y12, postclip = identity_default3, delta2 = 0.5, projectResample, projectTransform, projectRotateTransform, cache, cacheStream;
...
function conicEqualAreaRaw(y06, y12) {
  ...
  project.invert = function(x2, y2) {
    ...
    return [l / n, asin((c5 - (x2 * x2 + r0y * r0y) * n * n) / (2 * n))];
  };
  return project;
}
...
function albersUsa_default() {
  ...
  albersUsa.translate = function(_) {
    ...
    alaskaPoint = alaska.translate([x2 - 0.307 * k2, y2 + 0.201 * k2]).clipExtent([[x2 - 0.425 * k2 + epsilon7, y2 + 0.12 * k2 + epsilon7], [x2 - 0.214 * k2 - epsilon7, y2 + 0.234 * k2 - epsilon7]]).stream(pointStream);
    hawaiiPoint = hawaii.translate([x2 - 0.205 * k2, y2 + 0.212 * k2]).clipExtent([[x2 - 0.214 * k2 + epsilon7, y2 + 0.166 * k2 + epsilon7], [x2 - 0.115 * k2 - epsilon7, y2 + 0.234 * k2 - epsilon7]]).stream(pointStream);
    return reset();
  };
...
function azimuthalRaw(scale2) {
  return function(x2, y2) {
    var cx = cos2(x2), cy = cos2(y2), k2 = scale2(cx * cy);
    if (k2 === Infinity)
      return [2, 0];
    return [
      k2 * cy * sin2(x2),
...
function azimuthalInvert(angle2) {
  return function(x2, y2) {
    var z = sqrt(x2 * x2 + y2 * y2), c5 = angle2(z), sc = sin2(c5), cc2 = cos2(c5);
    return [
      atan2(x2 * sc, z * cc2),
      asin(z && y2 * sc / z)
    ];
...
function maxYReduce(y2, c5) {
  return Math.max(y2, c5.y);
}
...
function place(b, a3, c5) {
  var dx = b.x - a3.x, x2, a22, dy = b.y - a3.y, y2, b22, d2 = dx * dx + dy * dy;
  if (d2) {
    ...
    if (a22 > b22) {
      ...
      c5.x = b.x - x2 * dx - y2 * dy;
      c5.y = b.y - x2 * dy + y2 * dx;
    } else {
      x2 = (d2 + a22 - b22) / (2 * d2);
      y2 = Math.sqrt(Math.max(0, a22 / d2 - x2 * x2));
      c5.x = a3.x + x2 * dx - y2 * dy;
      c5.y = a3.y + x2 * dy + y2 * dx;
    }
  } else {
    c5.x = a3.x + c5.r;
    c5.y = a3.y;
...
function centroid_default3(polygon) {
  var i = -1, n = polygon.length, x2 = 0, y2 = 0, a3, b = polygon[n - 1], c5, k2 = 0;
  while (++i < n) {
    a3 = b;
    b = polygon[i];
...
var gamma_default = function sourceRandomGamma(source) {
  ...
  function randomGamma(k2, theta) {
    ...
    return function() {
      do {
        do {
          var x2 = randomNormal(), v2 = 1 + c5 * x2;
        } while (v2 <= 0);
        v2 *= v2 * v2;
        var u4 = 1 - source();
...
function transformer2() {
  ...
  function scale2(x2) {
    return x2 == null || isNaN(x2 = +x2) ? unknown : (output || (output = piecewise2(domain.map(transform2), range4, interpolate)))(transform2(clamp(x2)));
  }
  scale2.invert = function(y2) {
    return clamp(untransform((input || (input = piecewise2(range4, domain.map(transform2), number_default)))(y2)));
  };
  scale2.domain = function(_) {
...
function transformSymlog(c5) {
  return function(x2) {
    return Math.sign(x2) * Math.log1p(Math.abs(x2 / c5));
  };
}
function transformSymexp(c5) {
  return function(x2) {
    return Math.sign(x2) * Math.expm1(Math.abs(x2)) * c5;
  };
}
...
function transformer4() {
  var x06 = 0, x12 = 0.5, x2 = 1, s2 = 1, t03, t13, t22, k10, k21, interpolator = identity3, transform2, clamp = false, unknown;
  function scale2(x3) {
  ...
  scale2.domain = function(_) {
    return arguments.length ? ([x06, x12, x2] = _, t03 = transform2(x06 = +x06), t13 = transform2(x12 = +x12), t22 = transform2(x2 = +x2), k10 = t03 === t13 ? 0 : 0.5 / (t13 - t03), k21 = t13 === t22 ? 0 : 0.5 / (t22 - t13), s2 = t13 < t03 ? -1 : 1, scale2) : [x06, x12, x2];
  };
  scale2.clamp = function(_) {
    return arguments.length ? (clamp = !!_, scale2) : clamp;
  };
...
function pie_default() {
  ...
  function pie(data) {
    var i, n = (data = array_default3(data)).length, j, k2, sum4 = 0, index2 = new Array(n), arcs = new Array(n), a0 = +startAngle.apply(this, arguments), da2 = Math.min(tau6, Math.max(-tau6, endAngle.apply(this, arguments) - a0)), a1, p = Math.min(Math.abs(da2) / n, padAngle.apply(this, arguments)), pa = p * (da2 < 0 ? -1 : 1), v2;
    for (i = 0; i < n; ++i) {
...
var star_default = {
  draw(context, size) {
    ...
    for (let i = 1; i < 5; ++i) {
      ...
      context.lineTo(c5 * x2 - s2 * y2, s2 * x2 + c5 * y2);
    }
    context.closePath();
  }
};
...
function controlPoints(x2) {
  ...
  for (i = 1; i < n - 1; ++i)
    a3[i] = 1, b[i] = 4, r[i] = 4 * x2[i] + 2 * x2[i + 1];
